<div class="form-group mb-0 required">
    ...
    <div class="input-group">
        <select name="origin_id" id="origin_id"
            class="form-control {{ $errors->has('origin_id') ? ' is-invalid' : '' }}" onfocusout="validateCustomer()">
            <option class="dropup" value=""> Select One</option>
            @foreach ($places as $place)
            @if (old('origin_id') )
            <option class="dropup" value="{{$place->id}}" {{old('origin_id') == $place->id ? 'selected' : ''}}>
                {{$place->name}} </option>
            @else
            <option value={{$place->id}} {{ $place->id == $distance->origin_id ? 'selected' : ''}}>
                {{ $place->name }}</option>
            @endif
            @endforeach
...
<div class="form-group mb-2 required">
    ...
    <div class="input-group">
        <select name="destination_id" id="destination_id"
            ...
            <option class="dropup" value="{{$place->id}}" {{old('destination_id') == $place->id ? 'selected' : ''}}>
                {{$place->name}} </option>
            @else
            <option value={{$place->id}} {{ $place->id == $distance->destination_id ? 'selected' : ''}}>
                {{ $place->name }}</option>
            @endif
            @endforeach
        </select>
